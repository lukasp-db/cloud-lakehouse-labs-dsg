...
# MAGIC
# MAGIC **2. Amount Per Order**
# MAGIC ```
# MAGIC SELECT
# MAGIC     sum(total_amount)/sum(order_count) AS Amount_Per_Order
# MAGIC FROM churn_features
# MAGIC ```
# MAGIC Create a *counter* visualisation
# MAGIC
# MAGIC
# MAGIC **3. Customer Tenure - Historical**
# MAGIC ```
# MAGIC SELECT CAST(days_since_creation/30 AS INT) AS days_since_creation, gender, COUNT(*) AS customers
# MAGIC FROM churn_features
# MAGIC GROUP BY days_since_creation, gender
# MAGIC HAVING days_since_creation < 1000
# MAGIC ```
# MAGIC Create a *bar* visualisation
# MAGIC
# MAGIC **4. Subscriptions by Internet Service - Historical**
# MAGIC ```
# MAGIC select platform, channel, count(*) as event_count
# MAGIC from churn_app_events
# MAGIC inner join churn_users using (user_id)
# MAGIC where platform is not null
# MAGIC group by platform, channel
# MAGIC ```
# MAGIC Create a *heat map* visualisation
# MAGIC
# MAGIC **5. Users by channel**
# MAGIC ```
# MAGIC SELECT channel, count(*) as users
# MAGIC FROM churn_features
# MAGIC WHERE channel is not null
...
# MAGIC %md
# MAGIC
# MAGIC ## (Optional) Try using the new Lakeview Dashboards.
# MAGIC Navigate to the Dashboards tab, select Lakeview Dashboards tab, Create Lakeview Dashboard. Lakeview Dashboards represent a more modern experience. Data and credentials are stored with the dashboard allowing for easy sharing even outside of Databricks. Simply navigate from the Canvas to the Data tab, select the Churn Features table, and switch back to canvas. Try adding a few visualizations and filters on your own!
...
# MAGIC
# MAGIC Need to access cross-system data? Databricks SQL query federation let you define datasources outside of databricks (ex: Snowflake or BigQuery)
# MAGIC
...
# MAGIC This concludes the March 7th lab session. If you would like to do more activities see the Extra notebooks for ETL with DLT, Machine Learning, or Orchestration options
